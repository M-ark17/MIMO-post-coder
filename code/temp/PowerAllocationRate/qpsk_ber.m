...
max_bits_per_param = 1:2:12;
#max_bits_per_param = [1,3];
SNR = 0:2:20;# in decibels
#SNR = 18;# in decibels
Noise_power = 10^0.1; #in linear scale
...
for j = 1:length(snr)
    ...
    lambda = lambdaPower(diag(s),signal_power(j),Noise_power_db);
    p_sigma = getPower(receive_ant, transmit_ant, lambda);
#    u = generateU(receive_ant);
#    s = zeros(receive_ant);
#    s(1) = 1;
#    p_sigma(1,1) = signal_power(j); 
    disp(["SNR = ", num2str(SNR(j))]);fflush(stdout);
    for jj = 2:length(max_bits_per_param) 
        ...
        for ii = 1:2:number_of_bits
            ...
            err_count += nnz(input_bits!=received_data(1,:))+nnz(input_bits!=received_data(2,:));
           # disp("output");disp(received_data);
	   # disp("input");disp(input_bits);
	   # disp("error");disp(nnz(input_bits!=received_data(1,:)));
            if(jj ==1)
    	        ...
    	        err_count_theory += nnz(input_bits!=received_data_theory(1,:))+nnz(input_bits!=received_data_theory(2,:));
    	    end
        end
        ber(jj) = err_count/(4*number_of_bits);
	if(jj == 2)
          ber_theory(j) = err_count_theory/(4*number_of_bits);
	end
    end
    ber_snr(j,:) = ber';
    disp(["Finished BER calculation for SNR ",num2str(SNR(j))," ... "]);
    fflush(stdout);
end
