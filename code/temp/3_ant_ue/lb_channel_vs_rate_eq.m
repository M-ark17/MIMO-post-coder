...
for oo = 1:size(m,2)
  ...
  for ll=1:length(bits)
    ...
    bitCombs1 = generatecombs(3,bits(ll)); 
    rate_actu = 0;
    rate_actu1 = 0;
    all_sig = 0;
    one_sig = 0;
    ...
    for avg = 1:avg_iter
      ...
      rate_lb_with_one_sigma = zeros(1,size(bitCombs1,1));
	...
	rate_lb_with_all_sigma(kk) += rate_low;
      ...
      for kk = 1:size(bitCombs1,1)
	%%Finding lower bound if only one singular value is used
	rh1 = real(log2(1+P*sigma_lloyd_bound_reduced(K,bitCombs1(kk,1),bitCombs1(kk,2:end))));
    ...
    ind /= avg_iter;
    ind1 /= avg_iter;
    [num2str(3*bitCombs(ind,1)+2*bitCombs(ind,2)+bitCombs(ind,3)), "     ", num2str(all_sig/avg_iter), "    ", num2str(rate_actu/avg_iter), "     ", num2str(2*bitCombs1(ind1,1)+sum(bitCombs1(ind1,2:end))),"      ", num2str(one_sig/avg_iter), "     ", num2str(rate_actu1/avg_iter)] 
    fflush(stdout);
  end
end  
