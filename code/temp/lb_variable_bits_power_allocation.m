...
if(!exist ("avg_rate_files_mul_ant", "file"))
  mkdir "avg_rate_files_mul_ant";
end
...
for mm = 1:length(m)
  ...
  for ll=1:length(bits)
	...
	filename = ["avg_rate_files_mul_ant/" num2str(M) "ant" num2str(P_dB) "bits" num2str(bits(ll)) "sumsigma"	num2str(bits(ll)) "iterno" num2str(ii) ".mat"];
      else
	filename = ["avg_rate_files_mul_ant/" num2str(M) "ant" num2str(P_dB) "bits" num2str(bits(ll)) "sumsigma1_" num2str(bits(ll)) "iterno" num2str(ii) ".mat"];
      end
      if( exist(filename))
	...
	rate_lb(kk) = rate_lb(kk) + rate_low; 
      ...
      save(filename,"M",2bitCombs","rate_lb","rate_actu");
      disp(["bit iteration ",num2str(bits(ll)),"avg iteration",num2str(ii)]);
