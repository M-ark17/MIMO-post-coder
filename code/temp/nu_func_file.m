...
function s = sigma_lloyd_bound(r, b1, b2)
  s = r * (r - 1) * (1 - sinc(2**(-b1)));
  for l=1:r-1
    s = s + beta(2 / 3, l+1 / 3)**3 * l * (r-l) / 3 / 2**(2 * b2(l) + 3);
  end;
end;

function [epsilon] = epsillon_lloyd_error(r,b1,b)
  epsilon = 0;
  for l = 1:r-1
    epsilon += sqrt(1-(2*l)^-1)*(pi/2)^3*(r-l)/(12*2^(2*b(l)));
  end
  epsilon += r*(r-1)*(1-sinc(2^-b1));
end
